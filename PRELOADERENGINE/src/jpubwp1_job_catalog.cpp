...
int main(int argc, char** argv) {
	...
	if (argc == 3) {
		...
		if (argv1_string == "ADD") {
			prepare_query_WP1_JOB(argv[2]);
			commit_prepared_insert_WP1_JOB();
		} else if (argv1_string == "UPD") {
			prepare_query_WP1_JOB(argv[2]);
			commit_prepared_update_WP1_JOB();
		} else if (argv1_string == "DEL") {
			prepare_query_WP1_JOB(argv[2]);
			commit_deleted_WP1_JOB();
		} else if (argv1_string == "VIEW") {
			viewJobs(string(argv[2]));
		}else {
